...
El objetivo de esta solución es poder mezclar dos de las soluciones previas, usando la libreria de pandas para leer el archivo por lotes (chunks) 
y a la vez insertar en la base de datos utilizando el método `executemany()` de `psycopg2` , el cual permite insertar múltiples filas 
en una sola consulta. Igualmente a medida de que se itera sobre cada chunk, se aplica el filtro en el .csv para obtener los registros donde el 
primer campo del .csv PointOfSale contenga el carácter "_" y luego se va insertando en la tabla.
...
for chunk in pd.read_csv(args.archivo_csv, delimiter=';', chunksize=batch_size):
    ...
    # Construir una lista de tuplas con los valores de las filas filtradas
    values = [(row['PointOfSale'], row['Product'], row['Date'], row['Stock']) for _, row in filtered_chunk.iterrows()]

    # Insertar los datos filtrados en la tabla en una transacción
